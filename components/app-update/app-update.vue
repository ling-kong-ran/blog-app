	...
	export default {
		...
		methods: {
			...
			getSystemInfo() {
				...
				console.log('app版本号为：'+vm.version)
			...
			getUpdateInfo(){
				...
				uni.request({
				url: 'http://mrling.top:9898/app?httpData='+httpData, //仅为示例，并非真实接口地址。
				success: (res) => {
					let data = res.data;
					console.log(data)
					vm.update_info.type = 2
					vm.update_info.version_note = data.versionInfo
					vm.update_info.version = data.version
					vm.update_info.download_url = data.downloadUrl
					console.log('服务器端版本'+vm.update_info.version)
